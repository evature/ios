...
@property (nonatomic, strong, readwrite) NSString *fullPathToFilex;
...
void produceDataAsync(void* context) {
    MOAudioStreamer* slf = (MOAudioStreamer*)context;
    [slf produceData];
    if (!slf->orderToStop) {
        dispatch_async_f(slf->_streamDispatch, slf, produceDataAsync);
    }
}
...
- (void)startSend
{
    ...
    self.fullPathToFilex = [documentsDirectory stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.%@",self.fileToSaveName,ext ]];
    
#if DEBUG
    NSNumber *              fileLengthNum;
    fileLengthNum = (NSNumber *) [[[NSFileManager defaultManager] attributesOfItemAtPath:self.fullPathToFilex error:NULL] objectForKey:NSFileSize];
    if (fileLengthNum == NULL) {
        DLog(@"Error?  Null file size, path: %@   (not error if first run)", self.fullPathToFilex);
    }
    else {
        DLog(@"Start to send: file size %@ fullpath is %@",fileLengthNum,self.fullPathToFilex);
    }
#endif
...
//    [self.fileStream open];
    ...
    orderToStop=NO;
    dispatch_async_f(_streamDispatch, (void*)self, produceDataAsync);
...
-(void)removeRecordableFile{
    ...
    NSNumber *    fileLengthNum = (NSNumber *) [[[NSFileManager defaultManager] attributesOfItemAtPath:self.fullPathToFilex error:NULL] objectForKey:NSFileSize];
    // assert( [fileLengthNum isKindOfClass:[NSNumber class]] );
    if (fileLengthNum == NULL) {
        DLog(@"Error: Null file size!"); // could be ok if this is the first time
    }
    DLog(@"Removing: file size %@ fullpath is %@",fileLengthNum,self.fullPathToFilex);
