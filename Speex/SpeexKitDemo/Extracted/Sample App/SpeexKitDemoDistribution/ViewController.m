...
#define MIC_BUTTON_SPACE_OF_INTERNAL_CIRCLE 7//6//4
...
- (void)initBubbles{
    ...
    heyBubble.avatar = [UIImage imageNamed:@"people_juliane.png"];//@"evature.jpg"];//@"avatar1.png"];
...
-(void)userSay: (NSString *)userWords{
    ...
    NSBubbleData *sayBubble = [NSBubbleData dataWithText:userWords date:[NSDate dateWithTimeIntervalSinceNow:0] type:BubbleTypeMine];
    sayBubble.avatar = [UIImage imageNamed:@"Business_Person.png"];
...
-(void)evaSay: (NSString *)evaWords{
    ...
    NSBubbleData *sayBubble = [NSBubbleData dataWithText:evaWords date:[NSDate dateWithTimeIntervalSinceNow:0] type:BubbleTypeSomeoneElse];
    sayBubble.avatar = [UIImage imageNamed:@"people_juliane.png"];//@"evature.jpg"];
    [bubbleData addObject:sayBubble];
    [bubbleTable reloadData];
    
    
    [self.fliteController say:evaWords // @"A short statement"
                    withVoice:self.slt];
...
- (void) samplesAvailable:(SInt16 *)samples withNumberOfSamples:(int)numberOfSamples {
    ...
    if((numberOfBuffersMadeAvailable <= maxNumberOfBuffersToRecord)) { // But until that time, let's add the data we're going to convert to an array that will just consist of NSDatas of raw PCM samples, ready for Speex conversion.
